...
router.get(
  ...
  asyncMiddleware(async (req, res) => {
    const plantDocs = await Plant.find({ trefleid: { $exists: true, $ne: null } }).sort("name");
...
//Discover Label
router.post("/:id/discover-label", asyncMiddleware(async (req, res) => {
...
//trefle 
router.post("/:id/trefle-search", asyncMiddleware(async (req, res) => {
  const plant = await Plant.findById(req.params.id);
  if (!plant)
    return res.status(404).send("the plant with the given id was not found");

    if (!plant.label) 
    return res.status(400).send("no label was selected");
...
router.put("/:trefleid", async (req, res) => {
  ...
  if (!plant)
    return res.status(404).send("the plant with the given trefle id was not found");
